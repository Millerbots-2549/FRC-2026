...
public class ShooterCommands {
  ...
  public static Command homeShooter(Shooter shooter, DoubleSupplier distanceSupplier) {
    return Commands.run(() -> shooter.applyAutoAimDistance(distanceSupplier.getAsDouble()),shooter);
  }

  /**
   * Shoots balls according to the distance from the hub. Should be coupled with the point at hub command
   ...
   */
  public static Command shootWithHoming(Shooter shooter, Indexer indexer, DoubleSupplier distanceSupplier) {
    return new SequentialCommandGroup(
      // Gets rid of fuel stuck in shooter
      new ParallelCommandGroup(
        Commands.run(() -> shooter.setVelocity(FLYWHEEL_AGITATE_VELOCITY), shooter),
        Commands.run(() -> indexer.setRollerSpeed(-100), indexer)
      ).withTimeout(0.75),
      // Spins shooter up to speed, keeping fuel away while it gets up to speed
      new ParallelCommandGroup(
        Commands.run(() -> shooter.applyAutoAimDistance(distanceSupplier.getAsDouble()), shooter),
        Commands.run(() -> indexer.setRollerSpeed(-50), indexer)
